...
for kategori in KATEGORILER:
    ...
    # Haftalık aggregate — kategori × platform bazlı
    df['hafta'] = df['tarih'].dt.to_period('W-MON').apply(lambda x: x.start_time)
    
    haftalik = df.groupby(['hafta', 'platform']).agg(
...
# Kaydet
cikti = os.path.join(KLASOR, 'platform_haftalik_aggregate.xlsx')
sonuc.to_excel(cikti, index=False)

print(f"\nKaydedildi: {cikti}")
print(f"Toplam satır: {len(sonuc)}")
print()
print(sonuc.groupby(['kategori', 'platform'])['hafta'].count())